...
interface StoreContextType {
  ...
  addStockAdjustment: (adjustment: StockAdjustment) => Promise<void>;
  updateProductStock: (items: CartItem[], outletId?: string) => void;
...
const generateRandomId = (prefix: string) => `${prefix}-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;

export const StoreProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const handleSupabaseError = (error: unknown, operationType: string, path: string | null) => {
    console.error(`Supabase Error [${operationType}] on [${path}]:`, error);
    const err = error as Record<string, unknown>;
    const message = (err?.message as string) || (typeof error === 'object' ? JSON.stringify(error) : String(error));
    const errInfo = {
      error: message,
    ...
    console.error('Formatted Supabase Error: ', JSON.stringify(errInfo));
  };

    const addToSupabase = useCallback(async (tableName: string, data: Record<string, unknown>) => {
        if (!isSupabaseConfigured) return;
        try {
            const { error } = await supabase.from(tableName).insert(data);
            if (error) handleSupabaseError(error, 'insert', tableName);
        } catch (error) {
            handleSupabaseError(error, 'insert', tableName);
        }
    }, []);

    const updateInSupabase = useCallback(async (tableName: string, id: string, data: Record<string, unknown>) => {
        if (!isSupabaseConfigured) return;
        try {
            const { error } = await supabase.from(tableName).update(data).eq('id', id);
            if (error) handleSupabaseError(error, 'update', tableName);
        } catch (error) {
    ...
    const deleteFromSupabase = async (tableName: string, id: string) => {
        ...
        try {
            ...
            if (error) handleSupabaseError(error, 'delete', tableName);
        } catch (error) {
  ...
  const [stockAdjustments, setStockAdjustments] = usePersistentState<StockAdjustment[]>('app_stock_adjustments', []);
        ...
        if (navigator.onLine) {
            ...
            if (queue.length > 0) {
                ...
                for (const tx of queue) {
                    try {
                        await addToSupabase('transactions', {
                             id: tx.id,
                             date: tx.date,
                             total: tx.total,
  ...
  useEffect(() => {
        ...
        const subscribeToTable = (tableName: string, setter: (data: unknown[]) => void, transform?: (doc: Record<string, unknown>) => unknown) => {
            const channel = supabase
              .channel(`${tableName}-changes`)
              .on('postgres_changes', { event: '*', schema: 'public', table: tableName }, async () => {
                   const { data } = await supabase.from(tableName).select('*');
                   if (data) {
                       setter(transform ? (data as Record<string, unknown>[]).map(transform) : data);
                   }
              })
              .subscribe();
        ...
        subscribeToTable('products', setProducts);
        subscribeToTable('transactions', setTransactions);
        subscribeToTable('attendance', setAttendanceHistory, (item) => ({
            ...item,
            ...
            totalHours: item.total_hours || item.totalHours,
            checkInLocation: item.check_in_location || item.checkInLocation,
            checkOutLocation: item.check_out_location || item.checkOutLocation
        }));
        subscribeToTable('system_logs', setSystemLogs);
        subscribeToTable('employees', setEmployees, (item) => ({
            ...item,
            baseSalary: item.base_salary || item.baseSalary,
            hourlyRate: item.hourly_rate || item.hourlyRate,
            isWarehousePIC: item.is_warehouse_pic || item.isWarehousePIC,
            outletId: item.outlet_id || item.outletId,
            deviceIp: item.device_ip || item.deviceIp
        }));
        subscribeToTable('users', setUsers, (item) => {
            const dbRole = (item.role as string || '').toLowerCase();
            ...
            return {
                ...item,
                role: normalizedRole,
                isApproved: (item.is_approved ?? item.isApproved) as boolean
            };
        });
        subscribeToTable('commissions', setCommissions, (item) => ({
            ...item,
            referrerId: item.referrer_id || item.referrerId,
            referredUserId: item.referred_user_id || item.referredUserId,
            transactionId: item.transaction_id || item.transactionId,
            createdAt: item.created_at || item.createdAt
        }));
        subscribeToTable('outlets', setOutlets);
        subscribeToTable('settings', (data) => {
            if (data && data.length > 0) {
                ...
                setAppSettings(prev => ({
                    ...
                    heroImageUrl: s.hero_image_url || s.heroImageUrl,
                    maintenanceMode: s.maintenance_mode ?? s.maintenanceMode,
                    allowNegativeStock: s.allow_negative_stock ?? s.allowNegativeStock,
                    requireLocationForLogin: s.require_location_for_login ?? s.requireLocationForLogin,
                    attendanceRadius: s.attendance_radius || s.attendanceRadius || 100,
                    maxDiscountPercentage: s.max_discount_percentage || s.maxDiscountPercentage || 10,
                    enableDebtPayment: s.enable_debt_payment ?? s.enableDebtPayment,
                    galleryImages: s.gallery_images || s.galleryImages || []
                }));
            }
        });
    ...
    }, [setAppSettings, setAttendanceHistory, setCommissions, setEmployees, setOutlets, setProducts, setSystemLogs, setTransactions, setUsers]);

  const approveUser = async (id: string) => {
    try {
        const user = users.find(u => u.id === id);
        await updateInSupabase('users', id, { is_approved: true });
        ...
        addSystemLog({
            ...
            role: Role.ADMIN,
            action: 'ACTION',
            details: `User ${user?.name || id} approved`,
  ...
  const addDelivery = async (delivery: Delivery) => {
      ...
      await addToSupabase('deliveries', {
          id: delivery.id,
  ...
  const updateDelivery = async (delivery: Delivery) => {
      ...
      await updateInSupabase('deliveries', delivery.id, {
          transaction_id: delivery.transactionId,
          vehicle_id: delivery.vehicleId,
          driver_id: delivery.driverId,
          status: delivery.status,
  ...
  const addMarketNote = async (note: MarketNote) => {
      ...
      await addToSupabase('market_notes', {
          id: note.id,
          user_id: note.userId,
  ...
  const deleteMarketNote = async (id: string) => {
      ...
      await deleteFromSupabase('market_notes', id);
  };

  const addPricePoint = async (point: PricePoint) => {
      setPricePoints(prev => [...prev, point]);
      await addToSupabase('price_points', {
          id: point.id,
  ...
  const addMarketSurvey = async (survey: MarketSurvey) => {
      ...
      await addToSupabase('market_surveys', {
          id: survey.id,
  ...
  const addWeighingLog = async (log: WeighingLog) => {
      ...
      await addToSupabase('weighing_logs', {
          id: log.id,
  ...
  }, [setProducts, addToSupabase]);

  const updateProduct = useCallback(async (product: Product) => {
      setProducts(prev => prev.map(p => p.id === product.id ? product : p));
      await updateInSupabase('products', product.id, {
          name: product.name,
          category: product.category,
          price: product.price,
          stock: product.stock,
  ...
  const addCattlePrice = async (price: CattlePrice) => {
      ...
      await addToSupabase('cattle_prices', {
          id: price.id,
  ...
  const checkInEmployee = async (id: string, status: 'Hadir' | 'Terlambat' | 'Absen' | 'Pulang', time: string, location?: string, ip?: string) => {
      ...
      if (isSupabaseConfigured) {
          try {
              ...
              if (attendanceData && status === 'Pulang') {
                await supabase.from('attendance').update({
                    check_out_time: time,
                    status: status,
                    check_out_location: location || ''
                }).eq('id', attendanceData.id);
              } else if (!attendanceData && status !== 'Pulang') {
  ...
  const addCattleType = async (type: CattleType) => {
      ...
      await addToSupabase('cattle_types', {
          id: type.id,
          name: type.name,
  ...
  const deleteCattleType = async (id: string) => {
      ...
      await deleteFromSupabase('cattle_types', id);
  };
  ...
  const updateAppSettings = async (settings: Partial<AppSettings>) => {
      ...
      await updateInSupabase('settings', 'main', payload);
  };
  ...
  const addGalleryItem = async (item: GalleryItem) => {
    ...
    await addToSupabase('gallery_items', {
        id: item.id,
        title: item.title,
  ...
  const updateGalleryItem = async (item: GalleryItem) => {
    ...
    await updateInSupabase('gallery_items', item.id, {
        title: item.title,
        subtitle: item.subtitle,
        image_url: item.imageUrl,
        content: item.content,
  ...
  const deleteGalleryItem = async (id: string) => {
    ...
    await deleteFromSupabase('gallery_items', id);
  };

  const addEmployee = useCallback(async (employee: Employee) => {
      setEmployees(prev => [employee, ...prev]);
      await addToSupabase('employees', {
          id: employee.id,
          name: employee.name,
          ...
          is_warehouse_pic: employee.isWarehousePIC || false,
          phone: employee.phone || '',
          outlet_id: employee.outletId || 'HEAD-OFFICE',
          device_ip: employee.deviceIp || ''
      });
  }, [setEmployees, addToSupabase]);

  const updateEmployee = useCallback(async (employee: Employee) => {
      setEmployees(prev => prev.map(e => e.id === employee.id ? employee : e));
      await updateInSupabase('employees', employee.id, {
          name: employee.name,
          division: employee.division,
          position: employee.position,
          status: employee.status,
  ...
  const addLoyaltyProgram = async (program: LoyaltyProgram) => {
    ...
    await addToSupabase('loyalty_programs', {
        id: program.id,
        title: program.title,
  ...
  const updateLoyaltyProgram = async (program: LoyaltyProgram) => {
    ...
    await updateInSupabase('loyalty_programs', program.id, {
        title: program.title,
        description: program.description,
        target_kg: program.targetKg,
        duration_months: program.durationMonths,
  ...
  const addCommission = async (commission: Commission) => {
    ...
    if (isSupabaseConfigured) {
        try {
            const { data: userSnap } = await supabase.from('users').select('*').eq('id', commission.referrerId).single();
            if (userSnap) {
                const newTotal = (userSnap.total_earnings || 0) + commission.amount;
  ...
  const addStockAdjustment = async (adjustment: StockAdjustment) => {
    ...
    setProducts(prev => prev.map(p => {
        if (p.id === adjustment.productId) {
            ...
            // If DAMAGED or ADJUSTMENT with positive qty, we might be reducing stock?
            // Usually, these are outputs.
            return {
                ...p,
                stock: p.stock - qty
            };
    ...
    // Update in Supabase
    await addToSupabase('stock_adjustments', {
        id: adjustment.id,
    ...
    if (product) {
        await updateInSupabase('products', product.id, {
            stock: product.stock - Number(adjustment.quantity)
        });
    }
    ...
    addSystemLog({
        id: `log-${Date.now()}`,
        action: 'STOK',
        details: `${adjustment.type}: ${adjustment.productName} (${adjustment.quantity} ${product?.unit || 'unit'})`,
        timestamp: new Date().toISOString(),
  ...
  const addTransaction = async (transaction: Transaction) => {
    ...
    if (transaction.items && transaction.items.length > 0) {
        updateProductStock(transaction.items as CartItem[], transaction.outletId);
    }
    ...
    try {
        const payload = {
            ...
            outlet_id: transaction.outletId || 'HEAD-OFFICE',
            shipping_cost: transaction.shippingCost || 0,
            is_delivery: transaction.isDelivery || false,
            due_date: transaction.dueDate || '',
            down_payment: transaction.downPayment || 0,
            bank_name: transaction.bankName || '',
            bank_ref: transaction.bankRef || ''
        };
        await addToSupabase('transactions', payload);
    } catch (err) {
        console.error('Supabase transaction sync failed:', err);
    }
  ...
  const payReceivable = async (id: string, amountPaid: number, collectorId?: string) => {
      ...
      await addToSupabase('debt_payments', {
          id: payment.id,
      ...
      if (isFullyPaid && invoiceId) {
          await updateInSupabase('transactions', invoiceId, { status: 'Selesai' });
      }
  };
  ...
  const transferStock = async (productId: string, quantity: number, toOutlet: string) => {
      ...
      if (product) {
          ...
          await updateInSupabase('products', productId, { stock: newStock });
      }
  ...
  const addEmployeeFinancial = async (record: EmployeeFinancial) => {
    ...
    await addToSupabase('employee_financials', {
        id: record.id,
  ...
  const addCattleOrder = async (order: CattleOrder) => {
    ...
    await addToSupabase('cattle_orders', {
        id: order.id,
  ...
  const updateCattleOrder = async (order: CattleOrder) => {
    ...
    await updateInSupabase('cattle_orders', order.id, {
        supplier_id: order.supplierId,
        order_date: order.orderDate,
        expected_arrival: order.expectedArrival,
        status: order.status,
  ...
  }, [setOutlets, addToSupabase]);

  const updateOutlet = useCallback(async (outlet: Outlet) => {
    setOutlets(prev => prev.map(o => o.id === outlet.id ? outlet : o));
    await updateInSupabase('outlets', outlet.id, {
        name: outlet.name,
        address: outlet.address,
        phone: outlet.phone,
        coordinates: outlet.coordinates,
  ...
  const addVisitRecord = async (record: VisitRecord) => {
      ...
      await addToSupabase('visit_records', {
          id: record.id,
  ...
  const addSystemLog = async (log: SystemLog) => {
      ...
      await addToSupabase('system_logs', {
          id: log.id,
  ...
  const updateProductStock = async (items: CartItem[], outletId?: string) => {
    setProducts(prevProducts => 
      prevProducts.map(prod => {
        const cartItem = items.find(item => item.id === prod.id || (item.name === prod.name && prod.outletId === outletId));
        if (cartItem) {
    ...
    for (const item of items) {
         ...
         if (product) {
             ...
             await updateInSupabase('products', item.id, { stock: newStock });
         }
    }
  };

  const initializeMeatProducts = useCallback(async () => {
      const meatProducts = [
          { name: 'Daging Murni', price: 140000, category: ProductCategory.MEAT, unit: 'Kg' },
          { name: 'HAS Luar', price: 160000, category: ProductCategory.MEAT, unit: 'Kg' },
  ...
  }, [addProduct, outlets, products, updateProduct]);

  const initializeEmployees = useCallback(async () => {
      ...
      // Sync divisions to Supabase
      if (isSupabaseConfigured) {
          try {
              for (const div of divisions) {
                  await supabase.from('divisions').upsert({ id: div, name: div });
              }
          } catch (err) {
  ...
  }, [addEmployee, addOutlet, divisions, employees, outlets, updateEmployee, updateOutlet]);
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        const fetchTable = async <T,>(tableName: string, setter: (data: T[]) => void, transform?: (data: Record<string, unknown>[]) => T[]) => {
            try {
        ...
        // Auto-initialize if data is empty
        const currentProducts = await supabase.from('products').select('id', { count: 'exact', head: true });
        if (currentProducts.count === 0) {
            await initializeMeatProducts();
        }

        const currentEmployees = await supabase.from('employees').select('id', { count: 'exact', head: true });
    ...
    fetchData();
  }, [initializeEmployees, initializeMeatProducts, setAppSettings, setAttendanceHistory, setCattleOrders, setCattlePrices, setCattleTypes, setCommissions, setCustomers, setDebtPayments, setDeliveries, setEmployeeFinancials, setEmployees, setExpenses, setGalleryItems, setLeads, setLoyaltyPrograms, setMarketNotes, setMarketSurveys, setOutlets, setPricePoints, setProducts, setSystemLogs, setTransactions, setUsers, setVehicles, setVisitRecords, setWeighingLogs]);
